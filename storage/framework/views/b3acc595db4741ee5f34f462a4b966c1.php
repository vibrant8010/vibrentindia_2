<section class="banner-section mx-lg-4 mx-sm-2 mx-1 mt-3">
  <div class="container-fluid">
      <div class="row">
        <!-- Column 1 -->
        <div class="col-xl-3 col-lg-3 col-md-3 col-sm-6 col-6 p-1">
          <div class="slideshow" id="slideshow1">
            <img
              src="<?php echo e(asset('images/wb1.jpg')); ?>"
              class="active"
            />
            <img
              src="<?php echo e(asset('images/wb2.jpg')); ?>"
            />
            <img
              src="<?php echo e(asset('images/wb3.jpg')); ?>"
            />
          </div>
        </div>

        <!-- Column 2 -->
        <div class="col-xl-3 col-lg-3 col-md-3 col-sm-6 col-6 p-1">
          <div class="slideshow" id="slideshow2">
            <img
              src="<?php echo e(asset('images/wb4.jpg')); ?>"
              class="active"
            />
            <img
              src="<?php echo e(asset('images/wb5.jpg')); ?>"
            />
            <img
              src="<?php echo e(asset('images/wb6.jpg')); ?>"
            />
          </div>
        </div>

        <!-- Column 3 (Duplicate of Column 1) -->
        <div class="col-xl-3 col-lg-3 col-md-3 col-sm-6 col-6 p-1">
          <div class="slideshow" id="slideshow3">
            <img
              src="<?php echo e(asset('images/wb6.jpg')); ?>"
              class="active"
            />
            <img
              src="<?php echo e(asset('images/wb1.jpg')); ?>"
            />
            <img
              src="<?php echo e(asset('images/wb2.jpg')); ?>"
            />
          </div>
        </div>

        <!-- Column 4 (Duplicate of Column 2) -->
        <div class="col-xl-3 col-lg-3 col-md-3 col-sm-6 col-6 p-1">
          <div class="slideshow" id="slideshow4">
            <img
              src="<?php echo e(asset('images/wb3.jpg')); ?>"
              class="active"
            />
            <img
              src="<?php echo e(asset('images/wb4.jpg')); ?>"
            />
            <img
              src="<?php echo e(asset('images/wb5.jpg')); ?>"
            />
          </div>
        </div>
      </div>
  </div>
  </section>

<?php /**PATH C:\xampp\htdocs\vibrant_trade\resources\views/components/banner.blade.php ENDPATH**/ ?>